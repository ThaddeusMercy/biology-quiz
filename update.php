<?php
  ...
  if(isset($_SESSION['key']))
  {
    ...
    {
      $name = $_POST['name'];
      $name= mysqli_real_escape_string($con, stripslashes(ucwords(strtolower($name))));
      $description = mysqli_real_escape_string($con, stripslashes($_POST['description']));
      $total = $_POST['total'];
      $sahi = $_POST['right'];
      $wrong = $_POST['wrong'];
      $id=uniqid();
      $q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$id','$name', '$description' , '$sahi' , '$wrong','$total', NOW())");
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'addqns' && $_SESSION['key']=='admin') 
    {
      ...
      for($i=1;$i<=$n;$i++)
      {
        ...
        $ocid=uniqid();
        $odid=uniqid();
        $a=mysqli_real_escape_string($con, stripslashes($_POST[$i.'1']));
        $b=mysqli_real_escape_string($con, stripslashes($_POST[$i.'2']));
        $c=mysqli_real_escape_string($con, stripslashes($_POST[$i.'3']));
        $d=mysqli_real_escape_string($con, stripslashes($_POST[$i.'4']));
        $qa=mysqli_query($con,"INSERT INTO options (qid, option, optionid) VALUES  ('$qid','$a','$oaid')") or die('Error61' . mysqli_error($con));
        ...
        $qd=mysqli_query($con,"INSERT INTO options (qid, option, optionid) VALUES  ('$qid','$d','$odid')") or die('Error64' . mysqli_error($con));
        $e=$_POST['ans'.$i];
        switch($e)
        {
